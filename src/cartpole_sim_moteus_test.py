...
TODO: IN THE SWINGUPCONTROLLER ALSO HAVE A MOTEUS HARDWARE INTERFACE
...
# NOTE: This version has a moteusHardware interface implemented here
class SwingUpAndBalanceCtrlrMoteus(VectorSystem):
    ...
    def DoCalcVectorOutput(self, context, cart_pole_state, unused, output):
        ...
        # TODO:
        '''
        implement moteusMotor here to take [x, xdot] from cart_pole_state to move
        the real furuta pendulum
        [ ] fxn to convert cartpole to furuta
            [ ] find radius of motor (r_motor)
            [ ] find circumference of motor (circ_motor)
            # NOTE: angular_vel = linear_vel / r_motor, equiv to xdot_furuta = xdot_cart / r_motor
                    angular_pos = linear_pos / r_motor, equiv to x_furuta = x_cart / r_motor
            # NOTE: x_furuta needs to be smaller than circ_motor
        [ ] find and implement position limits on furuta (to not kill the wires)
        '''
...
async def main():
    ...
    controller = builder.AddSystem(SwingUpAndBalanceCtrlrMoteus(cart_pole, cart_pole_context,
                                                               input_i, output_i,
                                                               Q, R, x_star))
